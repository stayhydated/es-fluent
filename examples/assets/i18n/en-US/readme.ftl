## GenderThisOnly

gender_this_only_this = Gender This Only

## LoginFormCombined

login_form_combined_this = Login Form Combined

## GenderChoice

gender_choice-Female = Female
gender_choice-Male = Male
gender_choice-Other = Other

## LoginError

login_error-InvalidPassword = Invalid Password
login_error-Something = Something { $f0 } { $f1 } { $f2 }
login_error-UserNotFound = User Not Found { $username }

## LoginFormCombinedDescriptionVariants

login_form_combined_description_this = Login Form Combined Description Variants
login_form_combined_description-password = Password
login_form_combined_description-username = Username

## LoginFormCombinedLabelVariants

login_form_combined_label_this = Login Form Combined Label Variants
login_form_combined_label-password = Password
login_form_combined_label-username = Username

## LoginFormVariantsDescriptionVariants

login_form_variants_description-password = Password
login_form_variants_description-username = Username

## LoginFormVariantsLabelVariants

login_form_variants_label-password = Password
login_form_variants_label-username = Username

## UserProfileChoice

user_profile_choice = User Profile Choice { $name } { $gender }

## UserProfileDerive

user_profile_derive = User Profile Derive { $name } { $gender }
