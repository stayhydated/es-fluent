...
1. **Resolve Bundled Keys**: In default mode, bundled key selection is delegated to the shared manager fallback logic, which uses ICU4X locale fallback plus `fluent-langneg` lookup instead of hand-rolled subtag stripping.
