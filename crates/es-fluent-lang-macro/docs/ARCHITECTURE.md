...
1. **Validate Languages**: Each subdirectory name is validated against a list of known BCP-47 language codes (`SUPPORTED_LANGUAGE_KEYS`). This file is generated by `cargo run -p xtask -- generate-lang-names`. Unrecognized language codes will cause a **compile-time error**.
