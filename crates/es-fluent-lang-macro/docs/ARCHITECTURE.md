...
1. **Read Configuration**: The macro reads `i18n.toml` from the crate manifest directory (using `es-fluent-toml`) to determine the `assets_dir` and `fallback_language`.
1. **Scan Assets**: It scans the configured assets directory for locale subdirectories.
