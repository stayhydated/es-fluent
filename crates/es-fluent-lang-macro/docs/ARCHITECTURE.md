...
The `#[es_fluent_language]` attribute macro generates a Rust enum representing the supported languages found in the project's configuration (`i18n.toml`) and locale-directory structure.
...
By default, the macro links the generated enum to the built-in `es-fluent-lang` runtime, which formats language names directly from ICU4X display-name data.
...
1. **No Base ID Link**: It does _not_ add `id = "es-fluent-lang"`.
