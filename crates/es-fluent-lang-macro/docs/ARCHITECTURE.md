...
1. **Validate Languages**: In default mode, each discovered locale is validated against a list of known bundled language keys (`SUPPORTED_LANGUAGE_KEYS`). This file is generated by `cargo xtask generate-lang-names`. Unsupported locales cause a **compile-time error** unless the macro is in custom mode.
1. **Resolve Bundled Keys**: In default mode, bundled key selection is delegated to the shared manager fallback logic, which walks the ICU4X/CLDR parent chain to find the nearest supported bundled key instead of using hand-rolled subtag stripping.
