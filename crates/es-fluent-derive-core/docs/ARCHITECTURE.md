...
3.  **Performance**: Reduces code bloat in the main runtime crate.
...
2.  **Validation (`src/validation.rs`)**: The parsed options are checked for semantic correctness (e.g., conflicting flags).
3.  **Naming (`src/namer.rs`)**: FTL keys and documentation strings are generated based on the validated options.
