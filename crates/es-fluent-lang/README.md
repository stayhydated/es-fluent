...
By default, language names are loaded from embedded per-locale resources under `i18n/<locale>/es-fluent-lang.ftl` (generated by `scripts/cldr-es-fluent-lang`). The runtime resolves fallback locales when needed (for example, `zh-CN` will fall back to `zh`).

When the `bevy` feature is enabled, the crate registers static resources for each locale found under `i18n/` at build time so Bevy can bundle language-name resources with your app.
