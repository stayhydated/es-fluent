...
deserializes inventory keys and variables into shared Fluent newtypes, keeps
expected keys typed through validation, and only stringifies them when rendering
diagnostics. Duplicate expected keys in runner metadata are rejected at this
read boundary instead of being silently overwritten. Source file/line metadata
is also kept in shared source newtypes before formatting diagnostics.
