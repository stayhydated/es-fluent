...
#[test]
fn binary_init_help_describes_build_rs_update() {
    Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args(["es-fluent", "init", "--help"])
        .assert()
        .success()
        .stdout(predicate::str::contains(
...
#[test]
fn binary_init_help_describes_dioxus_runtime_requirements() {
    Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args(["es-fluent", "init", "--help"])
        .assert()
        .success()
        .stdout(predicate::str::contains(
            "Requires --manager dioxus and --update-cargo-toml",
        ));
}

#[test]
fn binary_init_help_describes_dry_run_updates() {
    Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args(["es-fluent", "init", "--help"])
        .assert()
        .success()
        .stdout(predicate::str::contains(
            "show files and manifest updates without making changes",
        ));
}

#[test]
fn binary_init_help_describes_force_build_rs_scope() {
    Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args(["es-fluent", "init", "--help"])
        .assert()
        .success()
        .stdout(predicate::str::contains(
            "build.rs is only patched when safe",
        ));
}

#[test]
fn binary_init_help_does_not_advertise_package_filter() {
    Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args(["es-fluent", "init", "--help"])
        .assert()
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("--package <PACKAGE>"));
}
...
fn public_cli_docs_keep_common_usage_sentences_readable() {
    ...
    let docs = [
        ...
        (
            "CLI skill reference",
            manifest_dir.join("../../.agents/skills/use-es-fluent/references/cli-workflow.md"),
        ),
...
fn binary_every_command_has_a_noninteractive_success_path() {
    ...
    std::fs::write(init_temp.path().join("Cargo.toml"), fixtures::CARGO_TOML)
...
#[test]
fn binary_json_commands_report_invalid_path_as_json() {
    let missing_path = "/definitely/missing/path";
    let cases: &[(&str, &[&str])] = &[
        ("check", &["--output", "json"]),
        ("sync", &["--locale", "fr-FR", "--output", "json"]),
        ("tree", &["--output", "json"]),
        ("doctor", &["--output", "json"]),
        ("status", &["--output", "json"]),
    ];
    ...
    for (command_name, args) in cases {
        let output = Command::cargo_bin("cargo-es-fluent")
            .expect("binary exists")
            .args(
                ["es-fluent", *command_name, "--path", missing_path]
                    .into_iter()
        ...
        let json: Value = serde_json::from_slice(&output).unwrap_or_else(|error| {
            panic!(
                "{command_name} stdout should be JSON only: {error}\n{}",
                String::from_utf8_lossy(&output)
            )
        });
        let json_text = json.to_string();
        assert!(
            json_text.contains("Failed to canonicalize root directory")
                && json_text.contains(missing_path),
            "{command_name} JSON should include the invalid path, got {json_text}"
        );
    }
}
...
#[test]
fn binary_package_filter_ignores_invalid_i18n_toml_in_unselected_sibling() {
    let temp = create_workspace_with_invalid_i18n_sibling();
    let workspace = temp.path().to_str().expect("workspace path");

    let fmt_output = Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args([
            "es-fluent",
            "fmt",
        ...
        .stdout
        .clone();
    let fmt_json: Value = serde_json::from_slice(&fmt_output).expect("fmt stdout is JSON only");
    assert_eq!(fmt_json["error_count"], 0);
    assert_eq!(fmt_json["files"][0]["path"], "a/i18n/en/a.ftl");

    let check_output = Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args([
            "es-fluent",
            "check",
    ...
    let check_json: Value =
        serde_json::from_slice(&check_output).expect("check stdout is JSON only");
    assert_eq!(check_json["crates_discovered"], 1);
    assert_eq!(check_json["crates_checked"], 0);
    assert_eq!(check_json["error_count"], 0);

    std::fs::create_dir_all(temp.path().join("a/i18n/fr")).expect("create selected target locale");
    std::fs::write(temp.path().join("a/i18n/fr/a.ftl"), "hello = Bonjour\n")
        .expect("write selected target ftl");
    let sync_output = Command::cargo_bin("cargo-es-fluent")
        ...
        .stdout
        .clone();
    let sync_json: Value = serde_json::from_slice(&sync_output).expect("sync stdout is JSON only");
    assert_eq!(sync_json["error_count"], 0);
    assert_eq!(sync_json["results"][0]["crate_name"], "a");
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .success()
        .stdout(predicate::str::contains(
            "Would create locale directory for es (a)",
        ))
        .stdout(predicate::str::contains("(b)").not())
        .stderr(predicate::str::is_empty());
    assert!(!temp.path().join("a/i18n/es").exists());

    let doctor_output = Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args([
            "es-fluent",
            "doctor",
    ...
    let doctor_json: Value =
        serde_json::from_slice(&doctor_output).expect("doctor stdout is JSON only");
    assert_eq!(doctor_json["crates_discovered"], 1);
    assert_eq!(doctor_json["error_count"], 0);
    assert!(
        doctor_json["issues"]
            .as_array()
            .is_some_and(|issues| { issues.iter().all(|issue| issue["crate_name"] == "a") })
    );
}

#[test]
fn binary_doctor_json_does_not_prepare_runner_or_cargo_output() {
    let temp = fixtures::create_workspace();
    std::fs::write(temp.path().join("src/lib.rs"), "pub mod i18n;\n").expect("write lib");
    std::fs::write(
    ...
    let json: Value = serde_json::from_slice(&output).expect("doctor stdout is JSON only");
    assert_eq!(json["crates_discovered"], 1);
    assert_eq!(json["error_count"], 0);
    assert!(
        !temp.path().join(".es-fluent").exists(),
        "doctor should not prepare runner metadata"
    );
    assert!(
        !temp.path().join("target").exists(),
        "doctor should not run Cargo"
    );
}
...
fn create_symlinked_custom_library_workspace() -> (assert_fs::TempDir, assert_fs::TempDir) {
    ...
    .expect("write i18n config");
    std::fs::write(outside.path().join("libsrc/lib.rs"), "pub fn marker() {}\n")
        .expect("write outside lib");
    std::fs::write(
        temp.path().join("i18n/en/runner-lib-link.ftl"),
        "hello = Hello\n",
    )
    .expect("write fallback ftl");
...
#[cfg(unix)]
#[test]
fn binary_generate_rejects_symlinked_custom_library_parent_path_before_runner() {
    let (temp, _outside) = create_symlinked_custom_library_workspace();
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("library target path"))
        .stderr(predicate::str::contains("symlinked library"))
        .stderr(predicate::str::contains("target path component"));

    assert!(
        !temp.path().join(".es-fluent").exists(),
        "generate should reject symlinked library targets before runner metadata"
    );
    assert!(
        !temp.path().join("target").exists(),
        "generate should reject symlinked library targets before Cargo runs"
    );
}
...
fn binary_generate_rejects_symlinked_i18n_module_before_runner() {
    ...
    std::fs::write(temp.path().join("i18n/en/test-app.ftl"), "hello = Hello\n")
        .expect("write fallback ftl");
    std::fs::write(outside.path().join("i18n.rs"), "define_i18n_module!();\n")
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .failure()
        .stderr(predicate::str::contains("generation path"))
        .stderr(predicate::str::contains("src/i18n.rs is a symlink"))
        .stderr(predicate::str::contains("real Rust module"))
        .stderr(predicate::str::contains("file"));

    assert!(
        !temp.path().join(".es-fluent").exists(),
        "generate should reject symlinked i18n modules before runner metadata"
    );
    assert!(
        !temp.path().join("target").exists(),
        "generate should reject symlinked i18n modules before Cargo runs"
    );
}

#[test]
fn binary_doctor_rejects_fallback_locale_path_as_file() {
    let temp = assert_fs::TempDir::new().expect("workspace tempdir");
    std::fs::create_dir_all(temp.path().join("src")).expect("create src");
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .assert()
        .failure()
        .stdout(predicate::str::contains("fallback locale directory"))
        .stdout(predicate::str::contains("not a directory"));
}

#[test]
fn binary_doctor_json_rejects_assets_dir_path_as_file() {
    let temp = assert_fs::TempDir::new().expect("workspace tempdir");
    std::fs::create_dir_all(temp.path().join("src")).expect("create src");
    std::fs::write(
        temp.path().join("Cargo.toml"),
    ...
    assert!(json["issues"].as_array().is_some_and(|issues| {
        issues.iter().any(|issue| {
            issue["severity"] == "error"
                ...
                && issue["help"].as_str().is_some_and(|help| {
                    help.contains("Assets path")
                        && help.contains("i18n")
                        && help.contains("not a directory")
                        && !help.contains(temp.path().to_str().expect("workspace path"))
                })
        })
    }));
}
...
#[test]
fn binary_doctor_json_rejects_locale_named_asset_path_as_file() {
    let temp = assert_fs::TempDir::new().expect("workspace tempdir");
    std::fs::create_dir_all(temp.path().join("src")).expect("create src");
    std::fs::create_dir_all(temp.path().join("i18n/en")).expect("create fallback locale");
    std::fs::write(
    ...
    assert!(json["issues"][0]["help"].as_str().is_some_and(|help| {
        help.contains("i18n/fr") && !help.contains(temp.path().to_str().expect("workspace path"))
    }));
}

#[test]
fn binary_doctor_json_rejects_ftl_path_as_directory() {
    let temp = assert_fs::TempDir::new().expect("workspace tempdir");
    std::fs::create_dir_all(temp.path().join("src")).expect("create src");
    ...
    assert!(json["issues"].as_array().is_some_and(|issues| {
        issues.iter().any(|issue| {
            issue["severity"] == "error"
                && issue["message"]
                    .as_str()
                    .is_some_and(|message| message.contains("FTL file layout"))
                && issue["help"].as_str().is_some_and(|help| {
                    help.contains("Expected FTL path to be a file")
                        && help.contains("i18n/en/test-app.ftl")
                        && !help.contains(temp.path().to_str().expect("workspace path"))
                })
        })
    }));
...
#[test]
fn binary_doctor_json_rejects_noncanonical_locale_named_asset_path_as_file() {
    let temp = assert_fs::TempDir::new().expect("workspace tempdir");
    std::fs::create_dir_all(temp.path().join("src")).expect("create src");
    std::fs::create_dir_all(temp.path().join("i18n/en")).expect("create fallback locale");
    std::fs::write(
    ...
    std::fs::write(temp.path().join("i18n/en/test-app.ftl"), "hello = Hello\n")
        .expect("write fallback ftl");
    std::fs::write(temp.path().join("i18n/en-us"), "not a directory\n").expect("write locale file");
    ...
    assert!(
        json["issues"][0]["message"]
            .as_str()
            .is_some_and(|message| message.contains("locale path 'en-us'"))
    );
    assert!(json["issues"][0]["help"].as_str().is_some_and(|help| {
        help.contains("i18n/en-us") && !help.contains(temp.path().to_str().expect("workspace path"))
    }));
}

#[test]
fn binary_doctor_json_reports_invalid_i18n_config() {
    let temp = assert_fs::TempDir::new().expect("workspace tempdir");
    let outside_name = format!(
        "{}-doctor-outside-assets",
        temp.path()
            .file_name()
            .expect("temp name")
            .to_string_lossy()
    ...
    assert!(
        json["issues"][0]["help"]
            .as_str()
            .is_some_and(|help| help.contains("Invalid assets_dir") && help.contains("crate root"))
    );
...
#[test]
fn binary_doctor_json_warns_when_i18n_module_is_missing() {
    let temp = fixtures::create_workspace();
    let workspace = temp.path().to_str().expect("workspace path");
    ...
    assert!(json["issues"].as_array().is_some_and(|issues| {
        issues.iter().any(|issue| {
            issue["severity"] == "warning"
                && issue["message"]
                    .as_str()
                    .is_some_and(|message| message.contains("src/i18n.rs is missing"))
                && issue["help"]
                    .as_str()
                    .is_some_and(|help| help.contains("cargo es-fluent init"))
        })
    }));
}

#[test]
fn binary_doctor_json_rejects_inline_i18n_module_without_generated_module_file() {
    let temp = assert_fs::TempDir::new().expect("workspace tempdir");
    std::fs::create_dir_all(temp.path().join("src")).expect("create src");
    std::fs::create_dir_all(temp.path().join("i18n/en")).expect("create fallback locale");
    std::fs::write(
    ...
    .expect("write manifest");
    std::fs::write(temp.path().join("src/lib.rs"), "pub mod i18n {}\n")
        .expect("write inline i18n module");
    std::fs::write(
        temp.path().join("i18n.toml"),
        "fallback_language = \"en\"\nassets_dir = \"i18n\"\n",
    )
        ...
        issues.iter().any(|issue| {
            issue["severity"] == "error"
                && issue["message"].as_str().is_some_and(|message| {
                    message.contains("src/lib.rs defines inline module `i18n`")
                })
                && issue["help"]
                    .as_str()
                    .is_some_and(|help| help.contains("pub mod i18n;"))
        }) && issues.iter().any(|issue| {
            issue["severity"] == "warning"
                && issue["message"]
                    .as_str()
...
#[test]
fn binary_doctor_json_rejects_undeclared_i18n_module_file() {
    let temp = assert_fs::TempDir::new().expect("workspace tempdir");
    std::fs::create_dir_all(temp.path().join("src")).expect("create src");
    std::fs::create_dir_all(temp.path().join("i18n/en")).expect("create fallback locale");
    std::fs::write(
    ...
    assert!(
        json["issues"][0]["message"]
            .as_str()
            .is_some_and(|message| message.contains("src/lib.rs does not declare module `i18n`"))
    );
    assert!(
        json["issues"][0]["help"]
            .as_str()
...
#[test]
fn binary_doctor_json_skips_build_rs_checks_without_manager_macro_module() {
    let temp = fixtures::create_workspace();
    std::fs::write(temp.path().join("src/lib.rs"), "pub mod i18n;\n").expect("declare i18n module");
    std::fs::write(temp.path().join("src/i18n.rs"), "pub fn marker() {}\n")
        .expect("write non-manager i18n module");
    std::fs::create_dir_all(temp.path().join("build.rs")).expect("create build.rs dir");
    let workspace = temp.path().to_str().expect("workspace path");
    ...
    assert!(json["issues"].as_array().is_some_and(|issues| {
        issues.iter().all(|issue| {
            !issue["message"]
                .as_str()
                .is_some_and(|message| message.contains("build.rs"))
        })
    }));
}

#[test]
fn binary_doctor_json_rejects_build_rs_path_as_directory() {
    let temp = fixtures::create_workspace();
    std::fs::write(temp.path().join("src/lib.rs"), "pub mod i18n;\n").expect("declare i18n module");
    std::fs::write(
        temp.path().join("src/i18n.rs"),
    ...
    assert!(json["issues"].as_array().is_some_and(|issues| {
        issues.iter().any(|issue| {
            issue["severity"] == "error"
                && issue["message"]
                    .as_str()
                    .is_some_and(|message| message.contains("build.rs exists but is not a file"))
                && issue["help"]
                    .as_str()
                    .is_some_and(|help| help.contains("track_i18n_assets"))
        })
    }));
}
...
#[cfg(unix)]
#[test]
fn binary_doctor_json_rejects_symlinked_i18n_module_path() {
    let temp = fixtures::create_workspace();
    let outside = assert_fs::TempDir::new().expect("outside tempdir");
    std::fs::write(
        outside.path().join("i18n.rs"),
    ...
    assert!(json["issues"].as_array().is_some_and(|issues| {
        issues.iter().any(|issue| {
            issue["severity"] == "error"
                && issue["message"]
                    .as_str()
                    .is_some_and(|message| message.contains("src/i18n.rs is a symlink"))
                && issue["help"]
                    .as_str()
                    .is_some_and(|help| help.contains("real Rust module file"))
        })
    }));
}
...
#[cfg(unix)]
#[test]
fn binary_doctor_json_rejects_symlinked_build_rs_path() {
    let temp = fixtures::create_workspace();
    let outside = assert_fs::TempDir::new().expect("outside tempdir");
    std::fs::write(temp.path().join("src/lib.rs"), "pub mod i18n;\n").expect("declare i18n module");
    std::fs::write(
    ...
    assert!(json["issues"].as_array().is_some_and(|issues| {
        issues.iter().any(|issue| {
            issue["severity"] == "error"
                && issue["message"]
                    .as_str()
                    .is_some_and(|message| message == "build.rs is a symlink")
                && issue["help"]
                    .as_str()
                    .is_some_and(|help| help.contains("real Rust build script file"))
        })
    }));
}
...
#[cfg(unix)]
#[test]
fn binary_doctor_json_rejects_symlinked_custom_library_parent_path() {
    let temp = assert_fs::TempDir::new().expect("workspace tempdir");
    let outside = assert_fs::TempDir::new().expect("outside tempdir");
    std::fs::create_dir_all(temp.path().join("i18n/en")).expect("create fallback locale");
    std::fs::create_dir_all(outside.path().join("libsrc")).expect("create outside lib src");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"doctor-lib-link\"\nversion = \"0.1.0\"\nedition = \"2024\"\n\n[lib]\npath = \"linked/lib.rs\"\n\n[dependencies]\nes-fluent-manager-embedded = \"0.16.0\"\n",
    )
    .expect("write manifest");
    std::fs::write(
        temp.path().join("i18n.toml"),
    ...
    .expect("write i18n config");
    std::fs::write(outside.path().join("libsrc/lib.rs"), "pub mod i18n;\n")
        .expect("write outside lib");
    std::fs::write(
        outside.path().join("libsrc/i18n.rs"),
        "es_fluent_manager_embedded::define_i18n_module!();\n",
    )
    .expect("write outside i18n module");
    std::os::unix::fs::symlink(outside.path().join("libsrc"), temp.path().join("linked"))
    ...
    assert!(json["issues"].as_array().is_some_and(|issues| {
        issues.iter().any(|issue| {
            issue["severity"] == "error"
                && issue["message"].as_str().is_some_and(|message| {
                    message == "library target path is not a real in-crate source path"
                })
                && issue["help"].as_str().is_some_and(|help| {
                    help.contains("linked/lib.rs")
                        && help.contains("symlinked library target")
                        && !help.contains(temp.path().to_str().expect("workspace path"))
                })
        })
    }));
...
#[cfg(unix)]
#[test]
fn binary_status_json_reports_symlinked_library_target_path_as_setup_error() {
    let (temp, _outside) = create_symlinked_custom_library_workspace();
    ...
    assert_eq!(json["crates_checked"], 0);
    assert!(json["setup_errors"][0].as_str().is_some_and(|message| {
        message.contains("library target path")
            && message.contains("linked/lib.rs")
            && message.contains("symlinked library target")
            && !message.contains(temp.path().to_str().expect("workspace path"))
    }));
    assert!(
        !temp.path().join(".es-fluent").exists(),
        "status should not prepare runner metadata after library target setup errors"
    );
    assert!(
        !temp.path().join("target").exists(),
        "status should not run Cargo after library target setup errors"
    );
}
...
fn binary_status_json_reports_undeclared_i18n_module_as_setup_error() {
    ...
    assert_eq!(json["crates_checked"], 0);
    assert!(json["setup_errors"][0].as_str().is_some_and(|message| {
        message.contains("src/lib.rs does not declare module `i18n`")
            && message.contains("pub mod i18n;")
            && !message.contains(temp.path().to_str().expect("workspace path"))
    }));
    assert!(
        !temp.path().join(".es-fluent").exists(),
        "status should not prepare runner metadata after i18n module setup errors"
    );
    assert!(
        !temp.path().join("target").exists(),
        "status should not run Cargo after i18n module setup errors"
    );
}
...
fn binary_status_json_reports_symlinked_i18n_module_as_setup_error() {
    ...
    .expect("write lib");
    std::fs::write(outside.path().join("i18n.rs"), "pub fn external() {}\n")
        .expect("write outside i18n module");
    std::os::unix::fs::symlink(
        outside.path().join("i18n.rs"),
        temp.path().join("src/i18n.rs"),
    ...
    assert_eq!(json["crates_checked"], 0);
    assert!(json["setup_errors"][0].as_str().is_some_and(|message| {
        message.contains("src/i18n.rs is a symlink")
            && message.contains("real Rust module file")
            && !message.contains(temp.path().to_str().expect("workspace path"))
    }));
    assert!(
        !temp.path().join(".es-fluent").exists(),
...
#[cfg(unix)]
#[test]
fn binary_check_json_reports_symlinked_library_target_path_as_setup_issue() {
    let (temp, _outside) = create_symlinked_custom_library_workspace();
    ...
    assert!(
        !temp.path().join(".es-fluent").exists(),
        "check should not prepare runner metadata after library target setup errors"
    );
    assert!(
        !temp.path().join("target").exists(),
        "check should not run Cargo after library target setup errors"
    );
}
...
fn binary_check_json_reports_undeclared_i18n_module_as_setup_issue() {
    ...
    .expect("write config");
    std::fs::write(temp.path().join("i18n/en/test-app.ftl"), "").expect("write fallback ftl");
    ...
    assert!(
        !temp.path().join(".es-fluent").exists(),
        "check should not prepare runner metadata after i18n module setup errors"
    );
    assert!(
        !temp.path().join("target").exists(),
        "check should not run Cargo after i18n module setup errors"
    );
}
...
#[test]
fn binary_init_rejects_dioxus_runtime_without_manifest_update() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"dioxus-no-update\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    )
    .expect("write manifest");
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("--update-cargo-toml"));
}

#[test]
fn binary_init_rejects_dioxus_runtime_without_manifest_update_before_path_validation() {
    Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args([
            "es-fluent",
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("--dioxus-runtime"))
        .stderr(predicate::str::contains("--update-cargo-toml"))
        .stderr(predicate::str::contains("/definitely/missing/path").not());
}

#[test]
fn binary_init_rejects_dioxus_runtime_without_manager_before_path_validation() {
    Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args([
            "es-fluent",
...
#[test]
fn binary_init_rejects_dioxus_runtime_with_non_dioxus_manager_before_path_validation() {
    Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args([
            "es-fluent",
        ...
        .failure()
        .stderr(predicate::str::contains("--dioxus-runtime"))
        .stderr(predicate::str::contains("--manager dioxus"))
        .stderr(predicate::str::contains("/definitely/missing/path").not());
}

#[test]
fn binary_init_rejects_dioxus_runtime_with_non_dioxus_manager_before_manifest_update_requirement() {
    Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args([
            "es-fluent",
        ...
        .stderr(predicate::str::contains("--dioxus-runtime"))
        .stderr(predicate::str::contains("--manager dioxus"))
        .stderr(predicate::str::contains("--update-cargo-toml").not())
        .stderr(predicate::str::contains("/definitely/missing/path").not());
}

#[test]
fn binary_init_rejects_list_argument_errors_before_path_validation() {
    Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args([
            "es-fluent",
...
#[test]
fn binary_init_rejects_fallback_locale_in_additional_locales_without_writing() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"fallback-in-locales\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    )
    .expect("write manifest");
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .failure()
        .stderr(predicate::str::contains(
            "--locales must not include the fallback locale 'en-US'",
        ));

    assert!(!temp.path().join("i18n.toml").exists());
    assert!(!temp.path().join("assets/locales/en-US").exists());
    assert!(!temp.path().join("assets/locales/fr").exists());
    assert!(!temp.path().join("src/i18n.rs").exists());
    assert!(!temp.path().join("src/lib.rs").exists());
}

#[test]
fn binary_init_rejects_blank_path_before_filesystem_validation() {
    Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args(["es-fluent", "init", "--path", "   "])
        .assert()
        .failure()
        .stderr(predicate::str::contains("workspace path must not be empty"))
        .stderr(predicate::str::contains("crate root does not exist").not());
}

#[test]
fn binary_init_accepts_dioxus_runtime_comma_separated_values_with_spaces() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"dioxus-space\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
...
#[test]
fn binary_init_reports_manifest_feature_updates_as_dependency_updates() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::create_dir_all(temp.path().join("src")).expect("create src");
    std::fs::write(temp.path().join("src/lib.rs"), "pub fn marker() {}\n").expect("write lib");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"dioxus-feature-update\"\nversion = \"0.1.0\"\nedition = \"2024\"\n\n[dependencies]\nes-fluent = \"0.16.0\"\nunic-langid = \"0.9\"\nes-fluent-manager-dioxus = \"0.7\"\n",
    )
    .expect("write manifest");
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .success()
        .stdout(predicate::str::contains(
            "Cargo.toml dependency updates were applied.",
        ))
        .stdout(predicate::str::contains("Cargo.toml dependencies were added.").not());

    let manifest = std::fs::read_to_string(temp.path().join("Cargo.toml")).expect("read manifest");
    assert!(
        manifest.contains("es-fluent-manager-dioxus = { version = \"0.7\", features = [\"ssr\"] }"),
        "{manifest}"
    );
}

#[test]
fn binary_init_rejects_empty_comma_separated_dioxus_runtime_entries_without_writing() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"empty-dioxus-runtime\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    )
    .expect("write manifest");
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .failure()
        .stderr(predicate::str::contains(
            "dioxus runtime values must not be empty",
        ))
        .stderr(predicate::str::contains("empty entries"))
        .stderr(predicate::str::contains("comma"))
        .stderr(predicate::str::contains("lists"));

    let manifest = std::fs::read_to_string(temp.path().join("Cargo.toml")).expect("read manifest");
    assert!(!manifest.contains("es-fluent"));
    assert!(!temp.path().join("i18n.toml").exists());
    assert!(!temp.path().join("assets/locales/en").exists());
    assert!(!temp.path().join("src/i18n.rs").exists());
    assert!(!temp.path().join("src/lib.rs").exists());
}

#[test]
fn binary_init_dry_run_reports_when_cargo_toml_dependencies_are_already_present() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::create_dir_all(temp.path().join("src")).expect("create src");
    std::fs::write(
        temp.path().join("Cargo.toml"),
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .stdout(
            predicate::str::contains(
                "Cargo.toml dependency updates were included in the dry-run preview.",
            )
            .not(),
        );
}
...
#[test]
fn binary_init_preserves_existing_dependency_versions() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::create_dir_all(temp.path().join("src")).expect("create src");
    std::fs::write(temp.path().join("src/lib.rs"), "pub fn marker() {}\n").expect("write lib");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"old-deps\"\nversion = \"0.1.0\"\nedition = \"2024\"\n\n[dependencies]\nes-fluent = \"0.1\"\nunic-langid = \"0.8\"\nes-fluent-manager-embedded = \"0.1\"\n",
    )
    .expect("write manifest");
    ...
    let manifest = std::fs::read_to_string(temp.path().join("Cargo.toml")).expect("read manifest");
    assert!(manifest.contains("es-fluent = \"0.1\""), "{manifest}");
    assert!(manifest.contains("unic-langid = \"0.8\""), "{manifest}");
    assert!(
        manifest.contains("es-fluent-manager-embedded = \"0.1\""),
        "{manifest}"
    );
}
...
#[cfg(unix)]
#[test]
fn binary_init_update_cargo_toml_rejects_symlinked_manifest_before_writing() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    let outside = assert_fs::TempDir::new().expect("outside tempdir");
    ...
    std::os::unix::fs::symlink(&outside_manifest, temp.path().join("Cargo.toml"))
    ...
    let manifest = std::fs::read_to_string(&outside_manifest).expect("read outside manifest");
    assert_eq!(
        manifest,
        "[package]\nname = \"manifest-link\"\nversion = \"0.1.0\"\nedition = \"2024\"\n"
    );
...
#[test]
fn binary_init_rejects_empty_comma_separated_locale_entries_without_writing() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"empty-init-locale\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    )
    .expect("write manifest");
    ...
    assert!(!temp.path().join("i18n.toml").exists());
    assert!(!temp.path().join("assets/locales/en").exists());
    assert!(!temp.path().join("assets/locales/fr-FR").exists());
    assert!(!temp.path().join("src/i18n.rs").exists());
    assert!(!temp.path().join("src/lib.rs").exists());
}

#[test]
fn binary_init_rejects_empty_comma_separated_namespace_entries_without_writing() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"empty-init-namespace\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    )
    .expect("write manifest");
...
#[test]
fn binary_init_dry_run_does_not_report_existing_directories_as_created() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::create_dir_all(temp.path().join("src")).expect("create src");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"dry-existing-dir\"\nversion = \"0.1.0\"\nedition = \"2024\"\n\n[lib]\npath = \"src/lib.rs\"\n",
    )
    .expect("write manifest");
    std::fs::write(temp.path().join("src/lib.rs"), "pub fn existing() {}\n").expect("write lib");

    let src_dir = temp.path().join("src");
    let locale_dir = temp.path().join("assets/locales/en");
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .stdout(
            predicate::str::contains(format!("Would create directory {}", src_dir.display())).not(),
        );
}

#[test]
fn binary_init_rejects_virtual_workspace_root() {
    let temp = assert_fs::TempDir::new().expect("workspace tempdir");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[workspace]\nmembers = [\"app\"]\nresolver = \"2\"\n",
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("not virtual"))
        .stderr(predicate::str::contains("workspace root"))
        .stderr(predicate::str::contains("--path <member-crate>"));
...
#[test]
fn binary_init_accepts_package_manifest_path_as_crate_root() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"manifest-path\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
...
#[test]
fn binary_init_missing_package_manifest_path_reports_package_root() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .stderr(predicate::str::contains("crate root does not exist").not())
        .stderr(predicate::str::contains("Cargo.toml/Cargo.toml").not());
...
#[test]
fn binary_init_accepts_bare_manifest_path_from_crate_root() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"bare-manifest-path\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    )
    .expect("write manifest");
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .args(["es-fluent", "init", "--path", "Cargo.toml"])
        .assert()
        .success();
...
#[test]
fn binary_init_respects_custom_library_target_path() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"custom-lib\"\nversion = \"0.1.0\"\nedition = \"2024\"\n\n[lib]\npath = \"lib.rs\"\n",
    ...
    .expect("write manifest");
    std::fs::write(temp.path().join("lib.rs"), "pub fn existing() {}\n").expect("write lib");
    ...
    let lib = std::fs::read_to_string(temp.path().join("lib.rs")).expect("read lib");
    assert!(lib.contains("pub fn existing() {}"));
    assert!(lib.contains("pub mod i18n;"));
    assert!(temp.path().join("i18n.rs").exists());
    assert!(!temp.path().join("src/lib.rs").exists());
    assert!(!temp.path().join("src/i18n.rs").exists());
}

#[test]
fn binary_init_rejects_custom_library_target_path_that_is_directory() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::create_dir_all(temp.path().join("src")).expect("create src");
    std::fs::write(
        temp.path().join("Cargo.toml"),
...
#[test]
fn binary_init_rejects_restricted_i18n_module_decl() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::create_dir_all(temp.path().join("src")).expect("create src");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"restricted-i18n\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    )
    .expect("write manifest");
    std::fs::write(
        temp.path().join("src/lib.rs"),
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("declares module `i18n`"))
        .stderr(predicate::str::contains("visibility"))
        .stderr(predicate::str::contains("pub mod i18n;"));

    let lib = std::fs::read_to_string(temp.path().join("src/lib.rs")).expect("read lib");
...
#[test]
fn binary_init_rejects_private_i18n_module_decl() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::create_dir_all(temp.path().join("src")).expect("create src");
    std::fs::write(
        temp.path().join("Cargo.toml"),
...
#[test]
fn binary_init_respects_commented_i18n_module_decl() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::create_dir_all(temp.path().join("src")).expect("create src");
    std::fs::write(
        temp.path().join("Cargo.toml"),
...
#[test]
fn binary_init_rejects_inline_i18n_module_decl() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::create_dir_all(temp.path().join("src")).expect("create src");
    std::fs::write(
        temp.path().join("Cargo.toml"),
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("inline module"))
        .stderr(predicate::str::contains("`i18n`"));
...
#[test]
fn binary_init_rejects_custom_library_target_path_outside_crate() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    let outside = temp
        .path()
        .parent()
        .expect("temp parent")
        .join("outside-lib-test.rs");
    let _ = std::fs::remove_file(&outside);
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"escape-lib\"\nversion = \"0.1.0\"\nedition = \"2024\"\n\n[lib]\npath = \"../outside-lib-test.rs\"\n",
    )
    .expect("write manifest");
...
#[cfg(unix)]
#[test]
fn binary_init_rejects_symlinked_custom_library_target_path_outside_crate() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    let outside = assert_fs::TempDir::new().expect("outside tempdir");
    let outside_lib_dir = outside.path().join("libsrc");
    std::fs::create_dir_all(&outside_lib_dir).expect("create outside lib dir");
    std::fs::write(outside_lib_dir.join("lib.rs"), "pub fn marker() {}\n")
        .expect("write outside lib");
    std::os::unix::fs::symlink(&outside_lib_dir, temp.path().join("linked"))
        .expect("create library path symlink");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"symlink-lib\"\nversion = \"0.1.0\"\nedition = \"2024\"\n\n[lib]\npath = \"linked/lib.rs\"\n",
    )
...
#[cfg(unix)]
#[test]
fn binary_init_rejects_symlinked_custom_library_target_path_inside_crate() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    let real_lib_dir = temp.path().join("real-src");
    std::fs::create_dir_all(&real_lib_dir).expect("create real lib dir");
    std::os::unix::fs::symlink(&real_lib_dir, temp.path().join("linked"))
        .expect("create library path symlink");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"symlink-lib\"\nversion = \"0.1.0\"\nedition = \"2024\"\n\n[lib]\npath = \"linked/lib.rs\"\n",
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .failure()
        .stderr(predicate::str::contains("[lib].path"))
        .stderr(predicate::str::contains("not symlinks"));
...
#[cfg(unix)]
#[test]
fn binary_init_rejects_symlinked_default_library_parent_path_inside_crate() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    let real_lib_dir = temp.path().join("real-src");
    std::fs::create_dir_all(&real_lib_dir).expect("create real lib dir");
    std::os::unix::fs::symlink(&real_lib_dir, temp.path().join("src"))
        .expect("create default library parent symlink");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"symlink-default-lib\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    )
    .expect("write manifest");
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("default library path"))
        .stderr(predicate::str::contains("not symlinks"));
...
#[test]
fn binary_init_rejects_i18n_module_as_custom_library_target_path() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"i18n-lib\"\nversion = \"0.1.0\"\nedition = \"2024\"\n\n[lib]\npath = \"src/i18n.rs\"\n",
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .failure()
        .stderr(predicate::str::contains("[lib].path"))
        .stderr(predicate::str::contains("generated i18n module"));
...
#[test]
fn binary_init_rejects_assets_dir_outside_crate() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    let outside_name = format!(
        "{}-outside-assets",
        temp.path()
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .args([
            ...
            temp.path().to_str().expect("workspace path"),
            "--assets-dir",
            &format!("../{outside_name}/i18n"),
        ])
        .assert()
        .failure()
        .stderr(predicate::str::contains("--assets-dir"))
        .stderr(predicate::str::contains("crate root"));
...
#[cfg(unix)]
#[test]
fn binary_init_rejects_symlinked_assets_dir_outside_crate() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    let outside = assert_fs::TempDir::new().expect("outside tempdir");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"asset-symlink-escape\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    )
    .expect("write manifest");
...
#[cfg(unix)]
#[test]
fn binary_init_rejects_symlinked_assets_dir_inside_crate() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::create_dir_all(temp.path().join("src")).expect("create src");
    std::fs::create_dir_all(temp.path().join("real-i18n")).expect("create real assets");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"asset-symlink-inside\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    )
    .expect("write manifest");
    std::fs::write(temp.path().join("src/main.rs"), "fn main() {}\n").expect("write main");
    ...
    assert!(!temp.path().join("i18n.toml").exists());
    assert!(!temp.path().join("real-i18n/en").exists());
    assert!(!temp.path().join("real-i18n/fr-FR").exists());
}

#[cfg(unix)]
#[test]
fn binary_init_rejects_symlinked_existing_fallback_locale_dir() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    let outside = assert_fs::TempDir::new().expect("outside tempdir");
    std::fs::create_dir_all(temp.path().join("src")).expect("create src");
    std::fs::create_dir_all(temp.path().join("i18n")).expect("create assets dir");
    ...
    assert!(!temp.path().join("i18n.toml").exists());
    assert!(!outside.path().join("fallback-locale-symlink.ftl").exists());
}

#[cfg(unix)]
#[test]
fn binary_init_force_rejects_symlinked_i18n_toml_without_writing_external_file() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    let outside = assert_fs::TempDir::new().expect("outside tempdir");
    let outside_config = outside.path().join("i18n.toml");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"init-symlink-config\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    )
    .expect("write manifest");
    std::fs::write(&outside_config, "outside = true\n").expect("write outside config");
    std::os::unix::fs::symlink(&outside_config, temp.path().join("i18n.toml"))
...
#[test]
fn binary_init_rejects_noncanonical_fallback_language_without_writing() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"noncanonical-fallback\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    )
    .expect("write manifest");
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("canonical BCP-47"))
        .stderr(predicate::str::contains("sc"));
...
#[test]
fn binary_init_dot_assets_rejects_locale_names_ignored_by_all_locale_scans() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"dot-assets-bin\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    ...
    assert!(!temp.path().join("en").exists());
}

#[test]
fn binary_init_dot_assets_rejects_fallback_locale_name_ignored_by_all_locale_scans() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"dot-assets-fallback\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    )
    .expect("write manifest");
...
#[test]
fn binary_init_dot_assets_all_locale_file_commands_ignore_crate_dirs() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"dot-assets\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    ...
    Command::cargo_bin("cargo-es-fluent")
        ...
        .assert()
        .success();
    std::fs::create_dir(temp.path().join("target")).expect("create cargo target dir");
    for dirname in ["bin", "dev", "doc", "lib", "man"] {
        std::fs::create_dir(temp.path().join(dirname)).expect("create project dir");
    }
    ...
    std::fs::write(temp.path().join("en/dot-assets.ftl"), "hello = Hello\n")
        .expect("write fallback ftl");
    let output = Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args([
    ...
    assert_eq!(json["results"][0]["locale"], "bin");
}

#[test]
fn binary_init_accepts_comma_separated_values_with_spaces() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"comma-space\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    ...
    assert!(temp.path().join("assets/locales/fr").is_dir());
    assert!(temp.path().join("assets/locales/zh-CN").is_dir());
    let config = std::fs::read_to_string(temp.path().join("i18n.toml")).expect("read config");
    assert!(config.contains("namespaces = [\"ui\", \"errors\"]"));
}

#[test]
fn binary_init_deduplicates_comma_separated_namespaces() {
    let temp = assert_fs::TempDir::new().expect("init tempdir");
    std::fs::write(
        temp.path().join("Cargo.toml"),
        "[package]\nname = \"namespace-dedup\"\nversion = \"0.1.0\"\nedition = \"2024\"\n",
    )
    .expect("write manifest");
    ...
    let config = std::fs::read_to_string(temp.path().join("i18n.toml")).expect("read config");
    assert!(config.contains("namespaces = [\"ui\", \"errors\"]"));
    assert!(!config.contains("ui\", \"errors\", \"ui"));
}
...
#[cfg(unix)]
#[test]
fn binary_tree_text_rust_links_rejects_symlinked_library_target_before_runner() {
    let (temp, _outside) = create_symlinked_custom_library_workspace();
    ...
    assert!(
        !temp.path().join(".es-fluent").exists(),
        "tree should reject symlinked library targets before runner metadata"
    );
    assert!(
        !temp.path().join("target").exists(),
        "tree should reject symlinked library targets before Cargo runs"
    );
}
...
fn binary_tree_text_rust_links_rejects_symlinked_i18n_module_before_runner() {
    ...
    assert!(
        !temp.path().join(".es-fluent").exists(),
        "tree should reject symlinked i18n modules before runner metadata"
    );
    assert!(
        !temp.path().join("target").exists(),
        "tree should reject symlinked i18n modules before Cargo runs"
    );
}
...
#[test]
fn binary_json_commands_report_missing_package_filter() {
    let temp = fixtures::create_workspace();
    let workspace = temp.path().to_str().expect("workspace path");

    let tree = Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args([
            "es-fluent",
            "tree",
        ...
        .stdout
        .clone();
    let tree_json: Value = serde_json::from_slice(&tree).expect("tree stdout is JSON only");
    assert_eq!(tree_json["crates"], Value::Array(Vec::new()));
    assert_eq!(tree_json["error_count"], 1);
    assert_eq!(tree_json["errors"][0]["crate_name"], "workspace");
    assert!(
        tree_json["errors"][0]["message"]
            .as_str()
            .is_some_and(|message| message.contains("missing-package"))
    );

    let sync = Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args([
            "es-fluent",
            "sync",
        ...
        .stdout
        .clone();
    let sync_json: Value = serde_json::from_slice(&sync).expect("sync stdout is JSON only");
    assert_eq!(sync_json["keys_added"], 0);
    assert_eq!(sync_json["error_count"], 1);
    assert!(
        sync_json["errors"][0]
            .as_str()
            .is_some_and(|message| message.contains("missing-package"))
    );
    assert_eq!(sync_json["results"], Value::Array(Vec::new()));

    let status = Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args([
            "es-fluent",
            "status",
        ...
        .stdout
        .clone();
    let status_json: Value = serde_json::from_slice(&status).expect("status stdout is JSON only");
    assert_eq!(
        status_json["workspace_warnings"],
        Value::Array(vec![Value::String(
            "no configured crate found matching package filter 'missing-package'".to_string()
        )])
    );

    let check = Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args([
            "es-fluent",
            "check",
        ...
        .stdout
        .clone();
    let check_json: Value = serde_json::from_slice(&check).expect("check stdout is JSON only");
    assert_eq!(
        check_json["workspace_warnings"],
        Value::Array(vec![Value::String(
            "no configured crate found matching package filter 'missing-package'".to_string()
        )])
    );

    let doctor = Command::cargo_bin("cargo-es-fluent")
        .expect("binary exists")
        .args([
            "es-fluent",
            "doctor",
        ...
        .stdout
        .clone();
    let doctor_json: Value = serde_json::from_slice(&doctor).expect("doctor stdout is JSON only");
    assert_eq!(doctor_json["crates_discovered"], 0);
    assert_eq!(doctor_json["error_count"], 0);
    assert_eq!(doctor_json["warning_count"], 1);
    assert_eq!(doctor_json["issues"][0]["severity"], "warning");
    assert_eq!(
        doctor_json["issues"][0]["message"],
...
#[test]
fn binary_json_commands_reject_blank_path_before_workspace_discovery() {
    let cases = [
        ("fmt", vec!["fmt", "--path", "   ", "--output", "json"]),
        ("check", vec!["check", "--path", "   ", "--output", "json"]),
        (
            "sync",
            vec!["sync", "--path", "   ", "--all", "--output", "json"],
        ),
        ("tree", vec!["tree", "--path", "   ", "--output", "json"]),
        (
            "doctor",
            vec!["doctor", "--path", "   ", "--output", "json"],
        ),
        (
            "status",
            vec!["status", "--path", "   ", "--output", "json"],
        ),
    ];
    ...
    for (command_name, args) in cases {
        ...
        let json_text = json.to_string();
        assert!(
            json_text.contains("workspace path must not be empty"),
            "{command_name} should report the blank path, got {json_text}"
        );
        assert!(
            !json_text.contains("canonicalize"),
            "{command_name} should reject --path before canonicalizing it, got {json_text}"
        );
    }
}
...
#[test]
fn binary_json_commands_report_invalid_i18n_config_with_relative_paths() {
    let temp = fixtures::create_workspace();
    std::fs::write(temp.path().join("i18n.toml"), "fallback_language = [\n")
        .expect("write invalid i18n config");
    let workspace = temp.path().to_str().expect("workspace path");
    let cases = [
        ("fmt", vec!["fmt", "--path", workspace, "--output", "json"]),
        (
            "sync",
            vec![
                "sync", "--path", workspace, "--locale", "fr-FR", "--output", "json",
            ],
        ),
        (
            "tree",
            vec!["tree", "--path", workspace, "--output", "json"],
        ),
        (
            "doctor",
            vec!["doctor", "--path", workspace, "--output", "json"],
        ),
        (
            "check",
            vec!["check", "--path", workspace, "--output", "json"],
        ),
        (
            "status",
            vec!["status", "--path", workspace, "--output", "json"],
        ),
    ];
    ...
    for (command, args) in cases {
        let output = Command::cargo_bin("cargo-es-fluent")
            ...
            .clone();
        let json: Value =
            serde_json::from_slice(&output).expect("command stdout should be JSON only");
        let message = match command {
            "fmt" | "sync" => json["errors"][0].as_str(),
            "tree" => json["errors"][0]["message"].as_str(),
            "doctor" | "check" => json["issues"][0]["help"].as_str(),
            "status" => json["setup_errors"][0].as_str(),
            _ => None,
        }
        .unwrap_or_else(|| panic!("{command} JSON should include an error message: {json}"));

        assert!(
            message.contains("i18n.toml"),
            "{command} error should mention i18n.toml: {message}"
        );
        assert!(
            !message.contains(workspace),
            "{command} JSON error should not include the absolute workspace path: {message}"
        );
    }
}
