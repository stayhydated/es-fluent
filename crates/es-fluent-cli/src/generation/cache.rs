...
//! - Source file content hashing (for staleness detection)
...
/// Cache of source file content hash for staleness detection.
///
/// Tracks the blake3 hash of all source file contents. Unlike mtime-based checks,
/// this correctly handles "save without changes" scenarios where mtime updates
/// but content remains the same.
#[derive(Debug, Serialize, Deserialize, Default)]
pub struct ContentCache {
    /// Combined hash of all source file contents
    pub content_hash: String,
    /// Mtime of runner binary when the hash was computed (for quick invalidation)
    pub runner_mtime: u64,
}

impl ContentCache {
    const CACHE_FILE: &'static str = "content_cache.json";
    ...
    pub fn compute_content_hash(src_dirs: &[&Path]) -> String {
        use blake3::Hasher;
