...
-   **Global Integration**: Seamlessly hooking into `es-fluent`'s global context so that standard `localize!` macros work within Bevy systems.
...
2.  **Loading**: Bevy loads all `.ftl` assets defined by registered modules.
3.  **Compilation**: `I18nBundle` creates `FluentBundle`s from loaded assets.
4.  **Localization**:
